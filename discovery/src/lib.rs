...
async fn avahi_task(
    ...
    let mut stream = bus.receive_name_owner_changed_with_args(
        &[(0, "org.freedesktop.Avahi")],
    ).await?;
    ...
    loop {
        ...
        'wait_avahi: {
            ...
            if let Ok(avahi_peer) = zbus::fdo::PeerProxy::new(&conn, "org.freedesktop.Avahi", "/").await {
        ...
        *entry_group = Some(avahi_server.entry_group_new().await?);
        ...
        let mut entry_group_state_stream = entry_group.as_mut().unwrap().receive_state_changed().await?;
