...
impl MprisPlayerService {
    ...
    async fn open_uri(&self, _uri: &str) -> zbus::fdo::Result<()> {
        Err(zbus::fdo::Error::NotSupported("OpenUri not supported".to_owned()))
    }
    ...
    async fn set_loop_status(&mut self, value: LoopStatus) -> zbus::fdo::Result<()> {
        ...
        match value {
            ...
            LoopStatus::Track => {
                return Err(zbus::fdo::Error::NotSupported("Player control not implemented".to_owned()));
            }
            LoopStatus::Playlist => {
                if let Some(spirc) = &self.spirc {
...
impl MprisEventHandler {
    pub async fn spawn(
        ...
        let mpris_task = MprisTask { player, connection, cmd_rx };

        let join_handle = tokio::spawn(mpris_task.run());

        Ok(MprisEventHandler { cmd_tx, join_handle })
    }
